...
fn main() -> ! {
    ...
    event::ApStaConnected::update_handler(move |event| {
        connections += 1;
        let mac_addr = mac_to_hex(event.mac());
        let s: &str = core::str::from_utf8(&mac_addr).unwrap();
        esp_println::println!("connected {}, mac: {}", connections, s);
    });
        ...
        if socket.is_connected() {
            ...
            if !time_out {
                ...
                socket.write(
                    ...
                    <html>\
                        <body>\
                            <h1>Hello Rust! <br> Hello esp-radio!</h1>\
                        </body>\
                    </html>\r\n\
                    ",
                    )
...
pub fn mac_to_hex(mac: &[u8]) -> [u8; 17] {
    assert_eq!(mac.len(), 6);

    let mut fixed_size: [u8; 6] = [0; 6];
    fixed_size.copy_from_slice(mac);
    let mut s : [u8;17] = [0; 17];
    let mut c = 0;
    for byte in fixed_size {
        let r = byte_to_hex(byte);
        s[c] = r[0];
        s[c+1] = r[1];
        if c+2 < 17 {
            s[c+2] = ':' as u8;
